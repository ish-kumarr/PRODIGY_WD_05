...
function convertTimeStamp(timestamp, timezone){
    ...
    const roundedTimezone = Math.round(convertTimezone); // round to nearest whole number
    ...
    const options = {
        ...
        hour: "numeric",
        minute: "numeric",
        timeZone: `Etc/GMT${roundedTimezone >= 0 ? "-" : "+"}${Math.abs(roundedTimezone)}`,
        hour12: true,
    }
...
function getWeather() {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${currCity}&appid=${API_KEY}&units=${units}`)
        ...
        .catch(error => {
            console.error(error);
            city.innerHTML = "City not found. Please try another search.";
            datetime.innerHTML = "";
